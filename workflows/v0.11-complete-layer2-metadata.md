...
- ⚠️ 51 empty `agencies.json` files (Layer 2) - TEMPLATES ONLY

**Example of current state** (`data/states/maine/agencies.json`):
```json
...
**Deliverable**: 6 completed `agencies.json` files
...
1. **Populated JSON file**: `data/states/{state}/agencies.json`
...
3. Populate `data/federal/agencies.json` (transparency_law section)
